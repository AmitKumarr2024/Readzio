...
const ContactReplyModal = ({ message, onClose }) => {
  ...
  // Controls frontend "done" status after success
  const [done, setDone] = useState(false);

  const handleSend = async () => {
    setDone(false); // reset done for each submission
    try {
      await dispatch(
        replyContactMessage({
      ...
      setDone(true); // stop loading on error
    }
  };

  // Show notification then close modal after delay
  useEffect(() => {
    if (notificationStatus) {
      const timeout = setTimeout(() => {
        dispatch(clearNotificationStatus());
        setDone(true); // mark done so loading bar stops
        onClose(); // close modal
      }, 1500);
    ...
    <>
      ...
      <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center">
        <div className="relative bg-white rounded-2xl p-6 w-full max-w-md shadow-xl">
          ...
          <h2 className="text-lg font-semibold text-gray-900 mb-4">Reply to Contact Message</h2>
