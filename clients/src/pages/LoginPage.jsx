...
const LoginPage = () => {
  ...
  const { loading, isAuthenticated } = useSelector((state) => {
    console.log("[LoginPage] Auth state:", { loading: state.auth.loading, isAuthenticated: state.auth.isAuthenticated });
    return state.auth;
  });
  ...
  const handleManualLogin = async (e) => {
    e.preventDefault();
    console.log("[LoginPage] Initiating manual login:", { email });
    try {
      const response = await dispatch(login({ email, password })).unwrap();
      console.log("[LoginPage] Manual login response:", { userId: response._id });
      toast.success("Login successful!");
      const query = new URLSearchParams(location.search);
      const redirectPath = query.get("redirect") || "/";
      navigate(redirectPath);
    } catch (err) {
      console.error("[LoginPage] Manual login failed:", {
        message: err?.message,
        stack: err?.stack,
  ...
  const handleGoogleLoginSuccess = async (credentialResponse) => {
    ...
    try {
      ...
      console.log("[LoginPage] Google login response:", { userId: response.user?._id });
              ...
              <form onSubmit={handleManualLogin} className="space-y-5">
                <div className="relative">
                  ...
                  <input
                    ...
                    value={email}
                    onChange={(e) => {
                      console.log("[LoginPage] Email input changed:", { email: e.target.value });
                ...
                <div className="relative">
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      console.log("[LoginPage] Toggling password visibility:", { showPassword: !showPassword });
