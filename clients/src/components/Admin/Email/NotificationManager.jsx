...
export default function NotificationManager() {
  ...
  const handleDeleteNotifications = async () => {
    setIsDeleting(true);
    try {
      const response = await fetch('/dailyMail/notifications', {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
      });
      const data = await response.json();
      if (!response.ok) throw new Error(data.message || 'Failed to delete notifications');
      dispatch(setNotificationStatus(`Deleted ${data.deletedCount} notifications`));
    } catch (error) {
  ...
  return (
    <div className="bg-background-light dark:bg-background-dark text-text-main-light dark:text-text-main-dark p-4 sm:p-6">
      ...
      <div className="mb-6">
        <p className="mb-4">Manually delete all notifications from the database.</p>
        <button
