...
const AdminLocationDashboard = lazy(() => import("../../location/AdminLocationDashboard"));
...
const StatModal = ({ type, count, onClose }) => {
  ...
  const bgClasses = {
    online: "bg-gradient-to-br from-green-200 dark:from-green-900/50 to-emerald-300 dark:to-emerald-800/50",
    offline: "bg-gradient-to-br from-gray-200 dark:from-gray-700/50 to-gray-300 dark:to-gray-600/50",
    total: "bg-gradient-to-br from-blue-200 dark:from-blue-900/50 to-indigo-300 dark:to-indigo-800/50",
    guest: "bg-gradient-to-br from-red-200 dark:from-red-900/50 to-pink-300 dark:to-pink-800/50",
  };
...
const formatTime = (seconds) => {
  if (typeof seconds !== "number" || isNaN(seconds) || seconds <= 0) return "0 hr 0 min 0 sec";
  ...
  if (secs || (!hrs && !mins)) parts.push(`${secs} sec${secs === 1 ? "" : "s"}`);
  return parts.join(" ");
};
  ...
  return (
    ...
    >
      ...
      <motion.div variants={itemVariants} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { type: "online", count: onlineUsersCount, icon: Users, color: "green", label: "Online Users" },
          { type: "offline", count: offlineUsers, icon: Users, color: "gray", label: "Offline Users" },
          { type: "total", count: totalUsers, icon: Users, color: "blue", label: "Total Users" },
          { type: "guest", count: guestUsersCount, icon: Users, color: "red", label: "Guest Users" },
            ...
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-lg sm:text-xl font-bold flex items-center gap-2 text-text-main-light dark:text-text-main-dark">
                  <Icon className={`w-5 h-5 sm:w-6 sm:h-6 text-${color}-600 dark:text-${color}-400`} /> {label}
                </h3>
                <CountUp
                  end={count}
                  duration={1}
      ...
      >
        <h2 className="text-2xl sm:text-3xl font-semibold mb-4 sm:mb-6 flex items-center gap-2 text-text-main-light dark:text-text-main-dark">
          <Users className="w-6 h-6 sm:w-8 sm:h-8 text-blue-600 dark:text-blue-400" /> All User Locations
        </h2>
        <ErrorBoundary
          fallback={
        ...
        >
          <Suspense fallback={<Loader2 className="w-8 h-8 animate-spin mx-auto text-blue-600 dark:text-blue-400" />}>
     ...
     Content */}
      ...
      >
        <h2 className="text-2xl sm:text-3xl font-semibold mb-4 sm:mb-6 flex items-center gap-2 text-text-main-light dark:text-text-main-dark">
          <BarChart className="w-6 h-6 sm:w-8 sm:h-8 text-blue-600 dark:text-blue-400" /> Site Insights
        
        </h2>

        {/* Date Range Filter */}
        <motion.div variants={itemVariants} className="mb-4 sm:mb-6 flex flex-col sm:flex-row gap-4">
          <input
            type="date"
            name="startDate"
          ...
          />
          {!isValidDateRange && (
            <p className="text-red-500 text-sm">Start date must be before or equal to end date</p>
          )}
        </motion.div>
        ...
        {analyticsLoading ? (
          <div className="text-center py-8 sm:py-12">
            ...
            <span className="text-text-main-light dark:text-text-main-dark">Loading...</span>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            {currentUser?.role === "admin" && post && (
              ...
              >
                <h3 className="text-lg sm:text-xl font-semibold mb-4 flex items-center gap-2 text-text-main-light dark:text-text-main-dark">
                  <FileText className="w-5 h-5 sm:w-6 sm:h-6 text-blue-600 dark:text-blue-400" /> Post Insights
                
                </h3>
                <div className="space-y-2 text-text-main-light dark:text-text-main-dark">
                  <p className="text-sm sm:text-base">
                    <span className="font-medium">Post Title:</span>{" "}
                    <span className="text-blue-600 dark:text-blue-400 font-bold">{post.title || "N/A"}</span>
                  </p>
                  <p className="text-sm sm:text-base">
                    <span className="font-medium">Your Read Time:</span>{" "}
                    <span className="text-blue-600 dark:text-blue-400 font-bold">{formatTime(sessionTime || 0)}</span>
                  </p>
                  <p className="text-sm sm:text-base">
                    <span className="font-medium">Total Time Spent:</span>{" "}
                    <span className="text-blue-600 dark:text-blue-400 font-bold">{formatTime(post.timeSpent || 0)}</span>
                  </p>
                  <p className="text-sm sm:text-base">
                    <span className="font-medium">Estimated Impressions:</span>{" "}
                    <span className="text-blue-600 dark:text-blue-400 font-bold">{Math.floor((post.timeSpent || 0) / IMPRESSION_INTERVAL)}</span>
                  </p>
                  <p className="text-sm sm:text-base">
                    <span className="font-medium">Ad Earnings (CPM $2.50):</span>{" "}
                    <span className="text-blue-600 dark:text-blue-400 font-bold">${calculateAdEarnings(post.timeSpent || 0)}</span>
                  </p>
                </div>
              </motion.div>
            )}
            ...
            >
              <h3 className="text-lg sm:text-xl font-semibold mb-4 flex items-center gap-2 text-text-main-light dark:text-text-main-dark">
                <Users className="w-5 h-5 sm:w-6 sm:h-6 text-blue-600 dark:text-blue-400" /> Traffic Overview
              </h3>
              <div className="space-y-2 text-text-main-light dark:text-text-main-dark">
                <p className="text-sm sm:text-base">
                  <span className="font-medium">Total Visits:</span>{" "}
                  <span className="text-blue-600 dark:text-blue-400 font-bold">{analytics.traffic?.totalVisits || 0}</span>
                </p>
                <p className="text-sm sm:text-base">
                  <span className="font-medium">Total Time Spent:</span>{" "}
                ...
                <p className="text-sm sm:text-base">
                  ...
                  <span className="text-blue-600 dark:text-blue-400 font-bold">{analytics.traffic?.uniqueUsersCount || 0}</span>
                </p>
                <p className="text-sm sm:text-base">
                  <span className="font-medium">Unique Posts:</span>{" "}
                  <span className="text-blue-600 dark:text-blue-400 font-bold">{analytics.traffic?.uniquePostsCount || 0}</span>
                </p>
                <p className="text-sm sm:text-base">
                  <span className="font-medium">Guest Users:</span>{" "}
                  <span className="text-blue-600 dark:text-blue-400 font-bold">{guestUsersCount || 0}</span>
                </p>
              </div>
            </motion.div>
            ...
            >
              <h3 className="text-lg sm:text-xl font-semibold mb-4 flex items-center gap-2 text-text-main-light dark:text-text-main-dark">
                <BarChart className="w-5 h-5 sm:w-6 sm:h-6 text-blue-600 dark:text-blue-400" /> Traffic Distribution
              </h3>
              <ResponsiveContainer width="100%" height={200}>
                <PieChart>
                  <Pie data={pieData} cx="50%" cy="50%" outerRadius={70} fill="#8884d8" dataKey="value" label>
            ...
            >
              <h3 className="text-lg sm:text-xl font-semibold mb-4 flex items-center gap-2 text-text-main-light dark:text-text-main-dark">
                <FileText className="w-5 h-5 sm:w-6 sm:h-6 text-blue-600 dark:text-blue-400" /> Top Posts
              </h3>
              {analytics.topPosts?.length > 0 ? (
                <ul className="space-y-3">
                  {analytics.topPosts.map((post) => (
                    <motion.li key={post.postId} variants={itemVariants} className="border-l-4 border-blue-500 pl-4">
                      <span className="font-medium text-text-main-light dark:text-text-main-dark">{post.title}</span>
                      <p className="text-sm text-text-main-light dark:text-text-main-dark">Visits: {post.visitCount || 0}</p>
                ...
                <p className="text-text-main-light dark:text-text-main-dark">No data available</p>
              )}
            </motion.div>
            ...
            >
              <h3 className="text-lg sm:text-xl font-semibold mb-4 flex items-center gap-2 text-text-main-light dark:text-text-main-dark">
                <Users className="w-5 h-5 sm:w-6 sm:h-6 text-blue-600 dark:text-blue-400" /> Top Active Users
              </h3>
              {Array.isArray(analytics.topUsers) && analytics.topUsers.length > 0 ? (
                <ul className="space-y-3">
                  {analytics.topUsers.map((user, index) =>
                    user?._id ? (
                      <motion.li key={user._id || index} variants={itemVariants} className="border-l-4 border-green-500 pl-4">
