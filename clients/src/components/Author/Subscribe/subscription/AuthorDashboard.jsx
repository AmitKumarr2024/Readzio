...
useEffect(() => {
  ...
  if (subError || postError) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-background-light dark:bg-background-dark text-text-main-light dark:text-text-main-dark">
        <motion.div
          className="p-8 bg-background-light dark:bg-background-dark text-text-main-light dark:text-text-main-dark rounded-2xl shadow-xl max-w-md w-full text-center"
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.5 }}
        >
          <p className="text-xl font-semibold text-red-600">
            Error: {subError || postError || "An error occurred"}
          </p>
        </motion.div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-background-light dark:bg-background-dark text-text-main-light dark:text-text-main-dark py-6 px-4 sm:px-6 lg:px-8">
      ...
      >
        {/* Sticky Tabs */}
        <div className="sticky top-0 z-10 bg-background-light dark:bg-background-dark text-text-main-light dark:text-text-main-dark backdrop-blur-xl rounded-2xl shadow-lg p-4 mb-6 border border-gray-100">
            ...
            {[
              ...
              <motion.button
                ...
                className={`flex items-center space-x-2 px-4 py-2 rounded-full font-medium text-sm transition-all duration-300 ${
                  activeTab === tab
                    ? "bg-gradient-to-r from-indigo-600 to-purple-600  text-text-main-light dark:text-text-main-dark shadow-md"
                    : " text-text-main-light dark:text-text-main-dark hover:bg-gray-100/80"
                }`}
                onClick={() => setActiveTab(tab)}
                whileHover={{
                  scale: 1.05,
                  backgroundColor:
                    activeTab === tab ? "transparent" : "#f3f4f6",
                }}
                whileTap={{ scale: 0.95 }}
              >
                {icon}
        ...
        <AnimatePresence mode="wait">
          <motion.div
            ...
            className="bg-background-light dark:bg-background-dark  backdrop-blur-xl rounded-2xl shadow-lg p-6 border border-gray-100"
          >
            {activeTab === "analytics" && (
              <PlansAnalytics
                userId={userId}
        ...
        <ModalBox isOpen={!!updatePlan} onClose={() => setUpdatePlan(null)}>
          <motion.div
            className="bg-background-light dark:bg-background-dark  rounded-2xl p-6"
            initial={{ scale: 0.7 }}
            animate={{ scale: 1 }}
