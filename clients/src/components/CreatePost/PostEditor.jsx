...
// External CSS for selection animations
...
const PostEditor = ({ title, setTitle, blocks, setBlocks, size }) => {
  ...
  // FIXED: Add block after selected index or at end
  const addBlock = (type, options = {}, afterIndex = null) => {
    ...
    if (newBlock) {
      ...
      // CRITICAL FIX: Insert after selected block or at end
      const newBlocks = [...blocks];
      const insertIndex = afterIndex !== null ? afterIndex + 1 : blocks.length;
      newBlocks.splice(insertIndex, 0, newBlock);

      setBlocks(newBlocks);
      toast.success(
        `${type.charAt(0).toUpperCase() + type.slice(1)} block added ${
          afterIndex !== null ? "after selected block" : "at end"
        }`
      );
      restoreScroll();
    }
  ...
  return (
    <>
      ...
      {/* FIXED: Added proper spacing to avoid sidebar overlap */}
      <div
        className={`min-w-[350px] ${sizeToWidthClass(
          size
        )} max-w-[1200px] min-h-[600px] sm:min-h-[800px] bg-background-light dark:bg-background-dark text-text-main-light dark:text-text-main-dark shadow-xl flex flex-col p-3 sm:p-4 md:p-6 rounded-2xl mx-auto transition-all duration-300 border border-gray-200 dark:border-gray-800 md:ml-20 ${
