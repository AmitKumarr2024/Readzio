...
import { getSubscriptionStatusByAuthor, checkEligibilityForSubscription } from '../../../store/subscriptionSlice';
...
const UserCardWrapper = ({ userId }) => {
  ...
  const followingList = useSelector((state) => state.follow?.following?.list || []);
  ...
  const handleFollowToggle = async () => {
    try {
      ...
      dispatch(getAllPosts({ followingIds, page: 1, limit: 12 }));
      refetchUserInfo();
    } catch (err) {
  ...
  if (loadingUser) return <div className="text-center py-4 animate-pulse">Loading user...</div>;
  ...
  const showButtons = userId && currentUser?._id && currentUser._id !== userToShow._id;
