    ...
    <div className="relative p-6 rounded-2xl bg-background-light dark:bg-background-dark text-text-main-light dark:text-text-main-dark max-w-md shadow-lg hover:shadow-xl transition-all transform hover:-translate-y-1 cursor-pointer group overflow-hidden">
      ...
      {isLoading ? (
        ...
        <Link
          ...
          className="flex items-center space-x-4 relative z-10 bg-background-light dark:bg-background-dark p-4 rounded-lg shadow-sm hover:bg-indigo-50 hover:scale-102 transition-all duration-300 group/link"
        >
          ...
          <div className="flex-1">
            ...
            <p className="text-sm">@{user.username || user.email || 'unknown'}</p>
            {user.email && <p className="text-xs mt-1">Email: {user.email}</p>}
          </div>
        </Link>
      )}
      {isLoading ? (
        ...
        <p className="text-sm mt-4 line-clamp-3 relative z-10">{user.bio || 'No bio available'}</p>
      )}
      {isLoading ? (
      ...
      {user.social && (
        <div className="flex flex-wrap gap-4 mt-6 text-sm relative z-10">
          {user.social.twitter && (
            <a href={user.social.twitter} className="text-indigo-500 hover:text-indigo-700" target="_blank" rel="noopener noreferrer">
              Twitter
            </a>
          )}
          {user.social.github && (
            <a href={user.social.github} className="text-indigo-500 hover:text-indigo-700" target="_blank" rel="noopener noreferrer">
              GitHub
            </a>
          )}
          {user.social.linkedin && (
            <a href={user.social.linkedin} className="text-indigo-500 hover:text-indigo-700" target="_blank" rel="noopener noreferrer">
              LinkedIn
            </a>
          )}
          {user.social.instagram && (
            <a href={user.social.instagram} className="text-indigo-500 hover:text-indigo-700" target="_blank" rel="noopener noreferrer">
              Instagram
            </a>
          )}
        </div>
