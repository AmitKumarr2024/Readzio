...
export const sendDailyPostEmail = async (req, res, next) => {
  ...
  try {
    ...
    // ✅ Rank by engagement and pick top 10
    posts = posts
      .sort(
        (a, b) =>
    ...
    for (let bi = 0; bi < batches.length; bi++) {
      ...
      const batchPromises = batches[bi].map(async (user) => {
        try {
          ...
          const mailOption = createMailOption({
            ...
            email: user.email,
            from: "Inksha <no-reply@inksha.com>", // ✅ Proper sender
            hasButton: true,
          ...
          await recordActivity({
            ...
            message: `Daily digest failed for ${user.email}: ${err.message}`,
    ...
    // ✅ Send admin report regardless of user failures
    ...
    if (admin?.email) {
      try {
        const adminMailOption = createMailOption({
          ...
          email: admin.email,
          from: "Inksha Reports <no-reply@inksha.com>",
        ...
        await sendEmailWithRetries(adminMailOption, admin._id, "report", 2);
    ...
    next(
      error instanceof AppError
        ? error
        : new AppError(error.message, 500, "SendDailyPostEmail")
    );
  }
};
