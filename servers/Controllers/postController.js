...
export const getSubscribedPosts = async (req, res, next) => {
  try {
    ...
    // Fetch user subscriptions — yeh example assume karta hai ki user model me subscriptions hain
    const user = await UserModel.findById(userId);

    if (!user) {
      throw new AppError("User not found", 404, "getSubscribedPosts Controller");
    }
    ...
    // Query posts by subscribed categories only
    ...
    next(new AppError(error.message, 500, "getSubscribedPosts Controller"));
  }
};
