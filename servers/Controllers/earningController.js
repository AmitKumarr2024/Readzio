...
const sendEmailWithRetries = async (mailOption, maxAttempts = 3) => {
  ...
  while (attempts < maxAttempts) {
    try {
      attempts++;
      console.log('Attempting to send email', { to: mailOption.to, attempt: attempts });
      await transporter.sendMail(mailOption);
      console.log('Email sent successfully', { to: mailOption.to, attempt: attempts });
      ...
      if (attempts < maxAttempts) {
        console.log('Retrying email send', { to: mailOption.to, attempt: attempts + 1 });
        await new Promise(resolve => setTimeout(resolve, 1000 * attempts)); // Exponential backoff: 1s, 2s, 3s
      }
    }
  }

  console.error('All email attempts failed', { to: mailOption.to, attempts, lastError: lastError.message });
...
export const getUserEarnings = asyncHandler(async (req, res, next) => {
  try {
    ...
    console.log('User earnings retrieved', { userId, totalEarnings: totalEarnings / 100 });
    ...
    console.error('Error fetching user earnings', { error: error.message, stack: error.stack });
...
export const getAllUsersEarnings = asyncHandler(async (req, res, next) => {
  try {
    ...
    console.error('Error fetching all users earnings', { error: error.message, stack: error.stack });
    next(new AppError(error.message, error.statusCode || 500));
  }
});
...
export const processBulkPayouts = asyncHandler(async (req, res, next) => {
  let session;
  try {
    console.log('Processing bulk payouts', { adminId: req.user?._id, users: req.body.users });
    if (!req.user?._id || req.user.role !== 'admin') {
      throw new AppError('Unauthorized: Admin access required', 403);
    }
    ...
    for (const userData of users) {
      ...
      let payoutResponse;
      const isRazorpayXMocked = process.env.NODE_ENV === 'development' ||
      ...
      if (!isRazorpayXMocked) {
        ...
        payoutResponse = await axiosInstance.post(
          '/payouts',
          razorpayPayout,
        ...
        await PaymentModel.findByIdAndUpdate(
          payout._id,
          { payoutId: payoutResponse.data.id, status: payoutResponse.data.status },
          { session }
        );
      } else {
        console.warn('Mocking RazorpayX payout');
        payoutResponse = { data: { id: `mock_payout_${Date.now()}`, status: 'queued' } };
      }
      ...
      if (shouldSendEmail) {
        const mailOption = createMailOption({
          ...
          name: user.name || 'User',
          email: user.email,
          message: `We have processed a payout of ₹${amount} to your account. Payout ID: ${payoutResponse.data.id}.\nPlease check your bank account for details.`,
          hasButton: false,
        });
        console.log('Preparing to send payout email', { mailOption, autoEmail: isAutoEmailDate() });
    ...
    session.endSession();
    console.log('Bulk payouts processed', { count: payouts.length, emailFailures });
    ...
    console.error('Error processing bulk payouts', { error: error.message, stack: error.stack });
    next(new AppError(error.message, error.statusCode || 500));
  }
});
...
export const recordSubscriptionPayment = asyncHandler(async (req, res, next) => {
  try {
    ...
    console.log('Recording subscription payment', { userId, orderId, amount, sendEmail });
    ...
    if (!orderId || !paymentId || !signature) {
      throw new AppError('Missing required fields: orderId, paymentId, or signature', 400);
    }
    ...
    if (sendEmail === 'true' || isAutoEmailDate()) {
      ...
      console.log('Preparing to send subscription payment email', { mailOption, autoEmail: isAutoEmailDate() });
    ...
    console.error('Error recording subscription payment', { error: error.message, stack: error.stack });
    next(new AppError(error.message, error.statusCode || 500));
  }
});
...
export const recordAdsPayment = asyncHandler(async (req, res, next) => {
  try {
    const { userId, amount, orderId, paymentId, signature, sendEmail } = req.body;
    console.log('Recording ads payment', { userId, orderId, amount, sendEmail });
    ...
    if (sendEmail === 'true' || isAutoEmailDate()) {
      ...
      console.log('Preparing to send ads payment email', { mailOption, autoEmail: isAutoEmailDate() });
    ...
    console.error('Error recording ads payment', { error: error.message, stack: error.stack });
    next(new AppError(error.message, error.statusCode || 500));
  }
});